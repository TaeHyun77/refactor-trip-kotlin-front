  ...
  const handleFileChange = async (e) => {
    ...
    if (file) {
      try {
        ...
        setProfileImage(`${response.data.url}?t=${new Date().getTime()}`);

        console.log("프로필 이미지 변경 중 에러 발생", response.data.url);
      } catch (error) {
        console.error("프로필 이미지 변경 중 에러 발생", error);
      }
    }
  };
          ...
          <form className="userInfo-form" onSubmit={onUpdate}>
            ...
            <div>
              <label htmlFor="createdDate">
                가입 날짜 : {formatDate(userInfo?.createdAt)}
              </label>
            </div>
